...
namespace Microsoft.VisualStudio.Services.Agent.Worker.Build
{
    ...
    {
        public string MaintenanceDescription => "Delete unused build directories.";
        ...
        public void RunMaintenanceOperation(IExecutionContext executionContext)
        {
            ...
            {
                staleBuildDirThreshold = Math.Min(staleBuildDirThreshold, 365);
                executionContext.Output($"Discovery stale build directories that haven't been used for more than {staleBuildDirThreshold} days.");

                // scan unused build directories
                trackingManager.DiscoverExpiredGarbage(executionContext, TimeSpan.FromDays(staleBuildDirThreshold));
            }
            else
            {
                executionContext.Output("Delete orphan and stale build directory option is not enabled.");
                return;
            }

            executionContext.Output("Delete orphan and stale build directory.");
